...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        try:
            km = KernelManager(kernel_name='coatl')
            km.start_kernel()
            ...
            print("Also, ensure 'coatl' kernel is installed for Jupyter.", file=sys.stderr)
            sys.exit(1)
        except Exception as e:
            print(f"\nAn unexpected error occurred: {e}", file=sys.stderr)
            print("Please ensure the 'coatl' kernel is correctly installed and registered with Jupyter.", file=sys.stderr)
            sys.exit(1)
        finally:
