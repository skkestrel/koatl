...
impl<'py> TlCtx<'py> {
    fn new(source: &'py str) -> TlResult<Self> {
        Ok(TlCtx { source })
    }
...
struct PyBlockWithFinal<'a> {
    stmts: SPyStmts<'a>,
    final_expr: Option<SPyExpr<'a>>,
}

struct PyExprWithPre<'a> {
    pre_stmts: SPyStmts<'a>,
    expr: SPyExpr<'a>,
}

trait SBlockExt<'a> {
    fn transform_with_final_stmt<'py>(&self, ast: &TlCtx<'py>) -> TlResult<SPyStmts>;
    fn transform_with_final_expr<'py>(&self, ast: &TlCtx<'py>) -> TlResult<PyBlockWithFinal>;
}

impl<'a> SBlockExt<'a> for SBlock<'a> {
    fn transform_with_final_stmt<'py>(&self, ctx: &TlCtx<'py>) -> TlResult<SPyStmts> {
        let (block, span) = self;
...
fn make_destructure_bindings<'a>(
    ctx: &TlCtx<'a>,
    target: &SExpr,
    decl_only: bool,
) -> TlResult<DestructureBindings<'a>> {
    let mut post_stmts = vec![];

    let assign_to: PyExpr = match &target.0 {
        Expr::Ident(..) | Expr::Attribute(..) | Expr::Subscript(..) => {
            ...
            let target_node = transpile_expr(ctx, target)?;
        ...
        Expr::List(items) => {
            /*
            Python destructure (issue: starred des goes into a list, not tuple)
            ...
            */
            ...
            let (block, cursor) = generate_list_destructure_ast(ctx, target, items)?;

            let stmts = block.transform_with_final_stmt(ctx)?;
            let curs = transpile_expr(ctx, &cursor)?;

            post_stmts.extend(stmts);
            post_stmts.extend(curs.pre_stmts);

            curs.expr
        }
        Expr::Mapping(items) => {
            let (block, cursor) = generate_mapping_destructure_ast(ctx, target, items)?;
...
trait SStmtExt<'a> {
    fn transform<'py>(&self, ctx: &TlCtx<'py>) -> TlResult<SPyStmts>;
}

impl<'a> SStmtExt<'a> for SStmt<'a> {
    fn transform<'py>(&self, ctx: &TlCtx<'py>) -> TlResult<SPyStmts> {
        let (stmt, span) = self;
        ...
        match stmt {
            Stmt::Expr(expr) => match &expr.0 {
                Expr::If(cond, then_block, else_block) => {
                    transpile_if_stmt(ctx, cond, then_block, else_block, span)
                }
                Expr::Match(subject, cases) => transpile_match_stmt(ctx, subject, cases, span),
                _ => {
                    let expr = transpile_expr(ctx, &expr)?;
                    let mut stmts = expr.pre_stmts;
                    stmts.push((PyStmt::Expr(expr.expr), Some(span)));

                    Ok(stmts)
                }
            },
            Stmt::Assert(expr, msg) => {
                let expr_node = transpile_expr(ctx, &expr)?;
                let msg = msg.as_ref().map(|x| transpile_expr(ctx, &x)).transpose()?;
                ...
                stmts.push(ctx.method1_unbound(
                    "Assert",
                    (expr_node.expr, msg_node),
                    Some(span),
                )?);
            ...
            Stmt::Return(expr) => {
                let value = transpile_expr(ctx, &expr)?;
                let mut stmts = value.pre_stmts;

                stmts.push(ctx.method1_unbound("Return", (value.expr,), Some(span))?);
            ...
            Stmt::Assign(target, value) => {
                if let (Expr::Ident((ident, ident_span)), Expr::Fn(arglist, body)) =
                    (&target.0, &value.0)
                {
                    return make_fndef_stmts(ctx, ident, &arglist, FnDefBody::Block(&body), span);
                }

                if let (Expr::Ident((ident, ident_span)), Expr::Class(bases, body)) =
                    (&target.0, &value.0)
                {
                    return make_classdef_stmts(ctx, ident, &bases, &body, span);
                }

                let value_node = transpile_expr(ctx, &value)?;
            ...
            Stmt::Global(names) => {
                let names: Vec<_> = names
                    .iter()
                    .map(|name| ctx.name(&name.0, PyAccessCtx::Store, Some(span)))
                    .collect::<TlResult<_>>()?;

                Ok(vec![ctx.method1_unbound("Global", (names,), Some(span))?])
            }
            Stmt::Nonlocal(names) => {
                let names: Vec<_> = names
                    .iter()
            ...
            Stmt::Raise(expr) => {
                ...
                stmts.push(ctx.method1_unbound("Raise", (expr_node.expr,), Some(span))?);

                Ok(stmts)
            }
            Stmt::For(target, iter, body) => {
                if let Expr::Ident((ident, _)) = &target.0 {
                    let iter_node = transpile_expr(ctx, iter)?;
                    let aux_stmts = iter_node.pre_stmts;

                    let body_block = transpile_block_with_final_stmt(ctx, body)?;

                    Ok(vec![ctx.method1_unbound(
                        "For",
                        (
                            ctx.name(ident, PyAccessCtx::Store, Some(span))?,
                            iter_node.expr,
                            body_block,
                            PyList::empty(ctx.py),
                        ),
            ...
            Stmt::While(cond, body) => {
                let cond_node = transpile_expr(ctx, cond)?;
                let body_block = transpile_block_with_final_stmt(ctx, body)?;

                let mut stmts = vec![];

                let cond = if cond_node.pre_stmts.is_empty() {
                    cond_node.expr
                } else {
                    ...
                    let aux_fn = make_fndef_stmts(
                        ...
                        &cond_name,
                        &vec![],
                        FnDefBody::Stmts(cond_node.pre_stmts),
                        span,
                    )?;
                    stmts.extend(aux_fn);
                    ...
                    ctx.method1_unbound(
                        "Call",
                        (
                            ctx.name(&cond_name, PyAccessCtx::Load, Some(span))?,
                            PyList::empty(ctx.py),
                            PyList::empty(ctx.py),
                        ),
                        Some(span),
                ...
                stmts.push(ctx.method1_unbound(
                    "While",
                    (cond, body_block, PyList::empty(ctx.py)),
                    Some(span),
                )?);
            ...
            Stmt::Try(body, excepts, finally) => {
                ...
                let finally_block = finally
                    .as_ref()
                    .map(|f| transpile_block_with_final_stmt(ctx, f))
                    .transpose()?;
                ...
                for except in excepts {
                    let type_node = if let Some(typ) = &except.typ {
                        let expr = transpile_expr(ctx, typ)?;
                        stmts.extend(expr.pre_stmts);
                        expr.expr
                    } else {
                        ctx.name("BaseException", PyAccessCtx::Load, Some(span))?
                    };

                    let ident_node = if let Some(ident) = &except.name {
                        PyString::new(ctx.py, &ident.0).into_any()
                    } else {
                        ctx.py.None().bind(ctx.py).clone()
                    };

                    let body_block = transpile_block_with_final_stmt(ctx, body)?;

                    let except_ast = ctx.method1_unbound(
                        "ExceptHandler",
            ...
            Stmt::Break => Ok(vec![ctx.method1_unbound("Break", (), Some(span))?]),
            Stmt::Continue => Ok(vec![ctx.method1_unbound("Continue", (), Some(span))?]),
            Stmt::Import(import_stmt) => {
                ...
                if import_stmt.star {
                    aliases.push(ctx.method1_unbound("alias", ("*", ctx.py.None()), Some(span))?);
                } else {
                    for (ident, alias) in &import_stmt.leaves {
                        aliases.push(ctx.method1_unbound(
                            "alias",
...
fn transpile_if_expr<'py>(
    ...
    let PyBlockWithFinal { stmts, final_expr } = transpile_block_with_final_expr(ast, then_block)?;
    ...
    let PyBlockWithFinal { stmts, final_expr } = transpile_block_with_final_expr(ast, else_block)?;
    ...
    for arg in arglist {
        match arg {
            ...
            ArgItem::DefaultArg(name, default) => {
                ...
                aux_stmts.append(&mut expr.pre_stmts);
...
trait SExprExt<'a> {
    fn transform<'py>(&self, ctx: &TlCtx<'py>) -> TlResult<PyExprWithPre>;
}

impl SExprExt<'_> for SExpr<'_> {
    fn transform<'py>(&self, ast: &TlCtx<'py>) -> TlResult<PyExprWithPre> {
        let no_aux = |expr| PyExprWithPre {
            expr,
            pre_stmts: vec![],
        };
        ...
        match expr {
            Expr::Fn(arglist, body) => {
                ...
                let aux_stmts =
                    make_fndef_stmts(ast, &name, arglist, FnDefBody::Block(body), span)?;
                ...
                for arg in args {
                    match &arg.0 {
                        ...
                        CallItem::ArgSpread(expr) => {
                            let e = transpile_expr(ast, &expr)?;
                            aux_stmts.extend(e.pre_stmts);
                            args_nodes.push(ast.method1_unbound(
                                "Starred",
                                (e.expr,),
                                Some(span),
                            )?);
            ...
            Expr::Block(block) => {
                let PyBlockWithFinal { stmts, final_expr } =
                    transpile_block_with_final_expr(ast, block)?;
            ...
            Expr::Slice(start, end, step) => {
                ...
                let mut get = |x: Option<PyExprWithPre>| {
                    if let Some(x) = x {
...
pub fn transform_ast<'a>(source: &str, block: &SBlock<'a>) -> TlResult<SPyStmts<'a>> {
    let ast = TlCtx::new(source)?;
    let span = &block.1;
